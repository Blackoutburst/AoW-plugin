...
public class Gunner extends Warrior {

	public Gunner(NPC npc, AowPlayer owner, Holo lifeBar, WarriorOptions options, Location position) {
	...
	protected void fight(Player p, int index) {
		...
		if (this.getOptions().combatDelay <= 0 && canFight(index)) {
			...
			for (int i = 0; i < 10; i++) {
				NMSParticle.spawnParticle(p, EnumParticle.FIREWORKS_SPARK, (float)(this.position.getX()), (float)(this.position.getY()) + 1.5f, (float)(this.position.getZ()) + (this.owner.getPlayerID() == 0 ?  (0.2f * i) : -(0.2f * i)));
			}
