...
public class Rifleman extends WarriorLogical {

	public Rifleman(AowPlayer owner, WarriorOptions options, Location position) {
	...
	protected void fight(int index) {
		...
		if (this.getOptions().combatDelay <= 0 && canFight(index)) {
			...
			for (AowPlayer p : Main.aowplayers) {
				NMSParticle.spawnParticle(p.getPlayer(), EnumParticle.EXPLOSION_NORMAL, (float)(op.getPosition().getX()), (float)(op.getPosition().getY()) + 1.5f, (float)(op.getPosition().getZ()));
