...
public class Cannoneer extends Warrior {

	public Cannoneer(NPC npc, AowPlayer owner, Holo lifeBar, WarriorOptions options, Location position) {
	...
	protected void fight(Player p, int index) {
		...
		if (this.getOptions().combatDelay <= 0 && canFight(index)) {
			...
			NMSParticle.spawnParticle(p, EnumParticle.EXPLOSION_LARGE, (float)(op.getPosition().getX()), (float)(op.getPosition().getY()), (float)(op.getPosition().getZ()));
